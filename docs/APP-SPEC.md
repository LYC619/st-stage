...
| `api` | API | 版本化连接档案快切：新建 Chat 档案提供 OpenAI、Claude、OpenRouter、Google AI Studio 与自定义 OpenAI 兼容入口，历史冷门渠道仍可编辑；Key 在扩展档案中明文保存并同步写入 ST 密钥库。ST 交互收敛在 `api/bridge.ts`（模拟器降级只读） | `st-extension/src/apps/api-app.ts` + `api/` |
