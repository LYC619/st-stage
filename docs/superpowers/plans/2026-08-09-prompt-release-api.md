...
- [ ] Add failing tests for first-enabled-pack bare-tag defaults, outfit/tag fallback, legacy role/tag precedence, and ambiguous fallback rejection.
- [ ] Implement the fallback without changing full three-part address parsing.
...
- [ ] Add a Seraphina-shaped fixture and assert one role header, one base pool, outfit increments, exact suffix lists, and a shorter result than grouped full mode.
- [ ] Treat `BUILTIN_TEMPLATE` equality as built-in behavior rather than custom-template mode.
- [ ] Render same-role scene labels as outfits and emit relative output rules consistent with Task 1.
- [ ] Preserve full mode and budget fitting, then run prompt tests.
...
- [ ] Add tests proving preview and injection receive identical notes and lengths.
- [ ] Reuse `buildPromptSceneNotes` in the App preview path.
- [ ] Change the built-in-template action/copy so an unchanged baseline keeps auto compression.
- [ ] Run focused App and entry tests.
...
- [ ] Add migration tests retaining plaintext profile keys and application tests that write the selected key before connection.
- [ ] Audit and test OpenAI/custom, OpenRouter, Claude, and Google AI Studio; route other OpenAI-schema services through custom.
- [ ] Keep existing imported profiles readable and avoid duplicating niche forms.
...
- [ ] Add/adjust build tests proving no image assets or development/reference files enter `st-distribution/`.
- [ ] Run focused tests after each task, then full Vitest, lint, typecheck, Next build, mobile E2E, and `git diff --check`.
- [ ] Rebuild root and distribution with one fixed stamp and compare shared SHA-256 hashes.
