...
project: st-stage
base_branch: main
verified_code_head: 3441d486eadda758bb8d051baee611141a4e45bc
remote_code_head_at_update: 55e0b981bcf27ab6503c8237b2eac40a9a28bed8
build_version: 0.9.0+202608100134
phase: acceptance-round2-awaiting-real-sillytavern
updated_at: 2026-08-10T01:38:00+08:00
updated_by: codex
verification_source: codex-acceptance-round2-2026-08-10
history: docs/maintenance/history/2026-08-10-acceptance-round2.md
---
...
- 第二轮验收修复已在 `codex/acceptance-round2` 分三批完成，代码提交依次为 `c8e629f`、`a0925e9`、`1238e00`；尚未合并或推送，`origin/main` 仍为 `55e0b98`。
- 产品版本仍为 `0.9.0`，构建戳为 `0.9.0+202608100134`。真实 SillyTavern 验收通过后再决定并执行 `1.0.0` 升版。
...
- 图库：同角色多层卡片堆叠与横向展开；批量上传、保存本地、复制分享串；安全删除元数据与未被引用的 `/user/images/` 文件。
- 资源：五套塞拉菲娜云端预设共 102 张，旧内置图片已从安装产物移除；`st-distribution/` 只保留 6 个安装文件。
- Prompt：默认服装和服装相对地址；角色级基础图名池、服装增量、`_变` 后缀与缩水集合；场景备注参与压缩；内置底稿未修改时仍自动精简；预览和注入共用同一构建入口。
...
Fresh automated evidence represented by non-maintenance head `3441d48` (implementation commit `1238e00`):

- Vitest：54 files, 625/625 tests passed.
...
10. Exercise batch reorder/delete. For local deletion, confirm only eligible `/user/images/` files are removed and files referenced by an unselected pack remain.
11. Exercise batch cloud upload, local save, and multi-share copying with both success and retryable failure cases.
12. Load all five hosted Seraphina presets, then save one locally and confirm subsequent rendering uses the ST user-image copy while sharing still retains the remote source.
...
15. Generate and resolve `[立绘:图名]`, `[立绘:服装/图名]`, and full `[立绘:角色/服装/图名]`; confirm ambiguous or wrong-role addresses do not render another pack's image.
